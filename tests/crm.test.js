...
describe('Automation — review tasks on Completed status', () => {
  ...
  test('creates one review task per admin when status changes to Completed', async () => {
    ...
    // At least one task for admin1 (server may also create one for the default nmora admin)
    expect(reviewTasks.length).toBeGreaterThanOrEqual(1);
    const admin1Task = reviewTasks.find((t) => t.leadOwner === adminId);
    expect(admin1Task).toBeDefined();
    expect(admin1Task.customerId).toBe('cust_comp');
    expect(admin1Task.title).toContain('Jane Doe');
    expect(admin1Task.completed).toBe(false);
  });
  ...
  test('creates tasks for each admin when multiple admins exist', async () => {
    ...
    const tasks = readJson(tasksFile, []);
    const reviewTasks = tasks.filter((t) => t.type === 'review_completed_app');
    // Expect tasks for both seeded admins (server may also add nmora)
    expect(reviewTasks.length).toBeGreaterThanOrEqual(2);
    const owners = reviewTasks.map((t) => t.leadOwner);
    expect(owners).toContain('admin1');
    expect(owners).toContain('admin2');
  });
  ...
  test('idempotency — does not duplicate tasks on repeated Completed transitions', async () => {
    ...
    // Capture the task count after the first transition
    const tasksAfterFirst = readJson(tasksFile, []);
    const countAfterFirst = tasksAfterFirst.filter(
      (t) => t.type === 'review_completed_app' && t.customerId === 'cust_idem'
    ).length;

    // Second call (still Completed — idempotent, no new tasks)
    await request(app)
      .put('/api/active-customers/cust_idem')
      .set('Authorization', `Bearer ${adminToken}`)
      .send({ status: 'Completed' });
    ...
    expect(reviewTasks.length).toBe(countAfterFirst);
  });
